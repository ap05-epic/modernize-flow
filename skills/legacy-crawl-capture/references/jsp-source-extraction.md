...
`extract_jsp.py` parses each JSP (and the `*.js` it references) into a `source-model.json` the driver agent
(implementation mode) implements from. The screenshot/DOM then only *verify* the result. This is what stops the agent from
...
```jsonc
{
  ...
  "forms":     [{"actions":["/<flow>.do"],
                 "fields":[{"tag":"html:text","property":"acct","type":"text"}]}], // -> controlled inputs, SAME name
  "ajaxEndpoints":[{"url":"/<ctx>/<flow>.do?tab=comp","via":"$.ajax url","source":"js/<flow>.js","context":"…"}],
  "messageKeys":[{"key":"<flow>.title"}],                   // exact copy (cross-ref the .properties bundle)
  "outputs":   [{"expr":"comp.total"}],                     // ${...} data bindings the screen shows
  "scripts":   ["js/<flow>.js"], "warnings": []
}
```
...
| `forms[].fields[].property/type` | controlled `<input name="acct">` — keep the **ActionForm name** |
